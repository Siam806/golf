...
function getBufferLower(category) {
	switch (category) {
		case 1:
			return 35; // Kat1: 35–36
		case 2:
			return 34; // Kat2: 34–36
		case 3:
			return 33; // Kat3: 33–36
		case 4:
			return 32; // Kat4: 32–36
		case 5:
			return 31; // Kat5: 31–36
		case 6:
			return 36; // Kat6 (vereinfacht)
		default:
			return 36;
	}
}
...
export default function ScoresRechner({ setega, setwhs }) {
	const { getWHS, saveRound } = useContext(authContext);
	...
	const saveCurrRound = () => {
		...
		// Speichere die Runde in LocalStorage
		saveRound(round);
	};
				...
				<div className="grid grid-cols-6 gap-2 p-4 bg-gray-800 rounded-lg">
					{holes.slice(0, holeCount).map((hole, index) => (
						<div key={index} className="flex flex-col items-center">
							...
							<input type="number" value={hole.par} onChange={(e) => handleHoleChange(index, "par", e.target.value)} className="p-2 text-white bg-gray-700 border rounded w-16 text-center" placeholder="Par" />
							<input type="number" value={hole.handicap} onChange={(e) => handleHoleChange(index, "handicap", e.target.value)} className="p-2 text-white bg-gray-700 border rounded w-16 text-center" placeholder="HCP" />
							<input type="number" value={hole.score} onChange={(e) => handleHoleChange(index, "score", e.target.value)} className="p-2 text-white bg-gray-700 border rounded w-16 text-center" placeholder="Schläge" />
						</div>
					))}
				</div>
